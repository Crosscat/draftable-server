...
describe('DraftService', () => {
  ...
  it('should create new draft', () => {
    ...
    service.joinNew({} as Player, { draftId: 'x' } as NewDraftRequest);

    expect(service.exists('x')).toBeTruthy();
  });

  it('should create new draft with unique id if none provided', () => {
    const draft = service.joinNew({} as Player, { } as NewDraftRequest);
  ...
  it('should start draft', () => {
    service.joinNew({ id: 'x' } as Player, { draftId: 'y' } as NewDraftRequest);
